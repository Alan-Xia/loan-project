      ...
      <el-table :data="list" style="width: 100%">
        ...
        <el-table-column label="操作">
          <template slot-scope="{row}">
            <el-button type="primary" size="small" :disabled="row.result == 'reject'" @click="queryLoan(row.loan_id)">查看</el-button>
            <el-button type="success" size="small" :disabled="row.result == 'reject'" @click="passInfo(row.id,row.loan_id)">通过</el-button>
            <el-button type="danger" size="small"  :disabled="row.result == 'reject'" @click="rejectInfo(row.id,row.loan_id)">拒绝</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
import {careQuery,queryLoanInfo,firstPass,firstReject} from '@/plugins/api'
const sexOptions = [
...
export default {
    ...
    methods: {
      ...
      passInfo (id,loan_id) {
        firstPass({appId:id,loanId:loan_id}).then(res => {
          if (res.code = '20000') {
            this.getFristCareData()
            this.$router.push({
              name: 'endCareFul'
      ...
      rejectInfo (id,loan_id) {
        firstReject({appId:id,loanId:loan_id}).then(res => {
          if (res.code = '20000') {
            this.getFristCareData()
            this.$notify({
              title: 'success',
